...
def split_and_save(df, output_path):
    ...
    original_test = test_df[['ID', 'Review_cleaned', 'Sentiment']].rename(columns = {'Review_cleaned': 'text'})

    neutral_train = train_df[['ID', 'text_all_neutral', 'Sentiment']].rename(columns = {'text_all_neutral': 'text'})
    neutral_test = test_df[['ID', 'text_all_neutral', 'Sentiment']].rename(columns = {'text_all_neutral': 'text'})
    ...
    mix_test = pd.concat([
        test_df[['ID', 'text_all_male', 'Sentiment']].rename(columns={'text_all_male': 'text'}),
        test_df[['ID', 'text_all_female', 'Sentiment']].rename(columns={'text_all_female': 'text'})])

    original_train.to_csv(os.path.join(output_path, "original_train.csv"), index = False)
    original_test.to_csv(os.path.join(output_path, "original_test.csv"), index = False)
    neutral_train.to_csv(os.path.join(output_path, "neutral_train.csv"), index = False)
    neutral_test.to_csv(os.path.join(output_path, "neutral_test.csv"), index = False)
    mix_train.to_csv(os.path.join(output_path, "mix_train.csv"), index = False)
    mix_test.to_csv(os.path.join(output_path, "mix_test.csv"), index = False)


def main():

    filepath = "/work/SofieNørboMosegaard#5741/NLP/NLP-exam/data/reviews.csv"
